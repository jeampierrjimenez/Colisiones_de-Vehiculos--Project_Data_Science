...
with tab1:
    sexo = st.selectbox(
        ...
        ('Male', 'Female', 'Ambos'))
    if Unknown == 'Si':
        st.plotly_chart(grafico_para_factores(df_factores, anio, sexo))
    else:
        st.plotly_chart(grafico_para_factores(df_factores_clean, anio, sexo))
with tab2:
    if Unknown == 'Si':
        st.plotly_chart(grafico_licencias_estados(df_licences, anio))
    else:
